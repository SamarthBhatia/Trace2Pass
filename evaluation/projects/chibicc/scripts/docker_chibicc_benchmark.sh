#!/bin/bash
# Trace2Pass Benchmark: chibicc (Make project, educational C compiler)
...
RUNS="${RUNS:-5}"
SOURCE_TARBALL="${SOURCE_TARBALL:-/data/project-sources/chibicc.tar.gz}"

echo "=== Trace2Pass chibicc Benchmark ==="
echo "LLVM version: $LLVM_VERSION"
echo ""

BENCH_SCRIPT=$(mktemp /tmp/bench_chibicc_XXXXXX.sh)
...
REPORTS_DIR="/reports"
mkdir -p /results /reports /workspace/chibicc
cd /workspace/chibicc

echo "[1/7] Downloading chibicc source..."
if [ -f /sources/chibicc.tar.gz ]; then
    tar xzf /sources/chibicc.tar.gz --strip-components=1
else
    apt-get update -qq && apt-get install -y -qq wget >/dev/null 2>&1
    wget -q "https://github.com/rui314/chibicc/archive/refs/heads/main.tar.gz" -O src.tar.gz
...
cp -r /workspace/chibicc /workspace/chibicc-baseline
cd /workspace/chibicc-baseline
make CC=clang CFLAGS="-O2" -j$(nproc) >/dev/null 2>&1 || \
    clang -O2 -o chibicc *.c -lm 2>/dev/null
BASELINE_SIZE=$(stat -c%s chibicc 2>/dev/null || echo 0)
cd /workspace/chibicc
...
make CC=clang \
    ...
    clang -O2 -fpass-plugin="$TRACE2PASS_PLUGIN" -o chibicc *.c \
    -L"$RUNTIME_DIR" -lTrace2PassRuntime -lm 2>/dev/null
...
INSTRUMENTED_SIZE=$(stat -c%s chibicc 2>/dev/null || echo 0)
...
cd /workspace/chibicc-baseline
./chibicc -o /dev/null /tmp/test_compile.c 2>/dev/null || true
...
cd /workspace/chibicc
export TRACE2PASS_OUTPUT="$REPORTS_DIR/chibicc_anomalies.json"
