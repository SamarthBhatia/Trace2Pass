#!/bin/bash
# Trace2Pass Benchmark: libpng (Autotools project)
...
RUNS="${RUNS:-5}"
SOURCE_TARBALL="${SOURCE_TARBALL:-/data/project-sources/libpng.tar.gz}"

echo "=== Trace2Pass libpng Benchmark ==="
echo "LLVM version: $LLVM_VERSION"
echo ""

BENCH_SCRIPT=$(mktemp /tmp/bench_libpng_XXXXXX.sh)
...
REPORTS_DIR="/reports"
mkdir -p /results /reports /workspace/libpng
cd /workspace/libpng

echo "[1/7] Downloading libpng source..."
if [ -f /sources/libpng.tar.gz ]; then
    tar xzf /sources/libpng.tar.gz --strip-components=1
else
    apt-get update -qq && apt-get install -y -qq wget >/dev/null 2>&1
    wget -q "https://downloads.sourceforge.net/libpng/libpng-1.6.44.tar.gz" -O src.tar.gz
    tar xzf src.tar.gz --strip-components=1
fi
echo "Done."

echo "[2/7] Installing build dependencies..."
apt-get update -qq && apt-get install -y -qq autoconf automake libtool zlib1g-dev bc >/dev/null 2>&1
echo "Done."

echo "[3/7] Building baseline (clang -O2)..."
cp -r /workspace/libpng /workspace/libpng-baseline
cd /workspace/libpng-baseline
CC=clang CFLAGS="-O2" ./configure --prefix=/workspace/libpng-baseline/install >/dev/null 2>&1
make -j$(nproc) >/dev/null 2>&1
make install >/dev/null 2>&1
BASELINE_SIZE=$(stat -c%s .libs/libpng16.a 2>/dev/null || echo 0)
cd /workspace/libpng
...
./configure --prefix=/workspace/libpng/install >/dev/null 2>&1
...
cd /workspace/libpng
export TRACE2PASS_OUTPUT="$REPORTS_DIR/libpng_anomalies.json"
