#!/bin/bash
# Trace2Pass Benchmark: mbedtls (CMake project)
...
RUNS="${RUNS:-5}"
SOURCE_TARBALL="${SOURCE_TARBALL:-/data/project-sources/mbedtls.tar.bz2}"

echo "=== Trace2Pass mbedtls Benchmark ==="
echo "LLVM version: $LLVM_VERSION"
echo ""

BENCH_SCRIPT=$(mktemp /tmp/bench_mbedtls_XXXXXX.sh)
...
REPORTS_DIR="/reports"
mkdir -p /results /reports /workspace/mbedtls
cd /workspace/mbedtls

echo "[1/7] Downloading mbedtls source..."
if [ -f /sources/mbedtls.tar.bz2 ]; then
    tar xjf /sources/mbedtls.tar.bz2 --strip-components=1
else
    apt-get update -qq && apt-get install -y -qq wget >/dev/null 2>&1
    wget -q "https://github.com/Mbed-TLS/mbedtls/releases/download/mbedtls-3.6.2/mbedtls-3.6.2.tar.bz2" -O src.tar.bz2
    tar xjf src.tar.bz2 --strip-components=1
fi
echo "Done."

echo "[2/7] Installing build dependencies..."
apt-get update -qq && apt-get install -y -qq cmake python3 bc >/dev/null 2>&1
echo "Done."

echo "[3/7] Building baseline (clang -O2)..."
mkdir -p build-baseline && cd build-baseline
cmake .. -DCMAKE_C_COMPILER=clang -DCMAKE_C_FLAGS="-O2" -DCMAKE_BUILD_TYPE=Release -DENABLE_TESTING=ON >/dev/null 2>&1
make -j$(nproc) >/dev/null 2>&1
BASELINE_SIZE=$(stat -c%s library/libmbedtls.a 2>/dev/null || echo 0)
cd ..
...
INSTRUMENTED_SIZE=$(stat -c%s library/libmbedtls.a 2>/dev/null || echo 0)
cd ..
echo "Done. Instrumented size: $INSTRUMENTED_SIZE bytes, compile: ${COMPILE_TIME_MS}ms"

echo "[5/7] Creating test workload..."
# mbedtls ships a benchmark program
echo "Using built-in selftest and benchmark programs."

echo "[6/7] Running baseline benchmark ($RUNS runs + 1 warmup)..."
# Warmup
build-baseline/programs/test/selftest >/dev/null 2>&1 || true

BASELINE_TOTAL=0
for i in $(seq 1 "$RUNS"); do
    START=$(date +%s%N)
    build-baseline/programs/test/selftest >/dev/null 2>&1 || true
    build-baseline/programs/test/benchmark >/dev/null 2>&1 || true
    END=$(date +%s%N)
...
export TRACE2PASS_OUTPUT="$REPORTS_DIR/mbedtls_anomalies.json"
export TRACE2PASS_SAMPLE_RATE=0.01

build-instrumented/programs/test/selftest >/dev/null 2>&1 || true

INSTRUMENTED_TOTAL=0
for i in $(seq 1 "$RUNS"); do
    START=$(date +%s%N)
    build-instrumented/programs/test/selftest >/dev/null 2>&1 || true
    build-instrumented/programs/test/benchmark >/dev/null 2>&1 || true
    END=$(date +%s%N)
    ELAPSED=$(( (END - START) / 1000000 ))
