...
download_project() {
    ...
    case "$proj" in
        sqlite)
            ...
            cd "$WORKDIR" && unzip -q sqlite.zip && mv sqlite-amalgamation-3470200 "$dest" && cd - >/dev/null
            ;;
        lz4)
        ...
        if [ -n "$SRC_FILES" ]; then
            ...
            if [ $ALL_OK -eq 1 ]; then
                ...
                if $CLANG $CF $INC_FLAGS -c "$HARNESS" -o "$BENCH_OBJ" 2>/dev/null; then
                    if $CLANG -O2 $LF $OBJ_FILES "$BENCH_OBJ" -o "$BIN" 2>/dev/null; then
                        BUILD_OK=1
                    fi
                fi
            fi
            ...
            if $CLANG $CF $INC_FLAGS -c "$HARNESS" -o "$WORKDIR/bench_${proj}_${label}.o" 2>/dev/null; then
                if $CLANG -O2 $LF "$WORKDIR/bench_${proj}_${label}.o" -o "$BIN" 2>/dev/null; then
                    BUILD_OK=1
                fi
            fi
        fi
