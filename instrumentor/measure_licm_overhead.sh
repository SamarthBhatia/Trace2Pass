...
echo "=== LICM Overhead Benchmark (100 iterations) ==="
echo

# Warm up
for i in {1..5}; do
    opt -passes="licm" test_licm.ll -o /dev/null 2>/dev/null
    opt -load-pass-plugin=./build/InstrumentedLICM.so -passes="instrumented-licm" test_licm.ll -o /dev/null 2>/dev/null
done

# Baseline: vanilla LICM
echo "Running vanilla LICM..."
...
# Instrumented
echo "Running instrumented LICM..."
